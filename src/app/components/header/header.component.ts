...
export class HeaderComponent implements AfterViewInit, CommonModule {
  ...
  animateClipPath(element: HTMLElement) {
    ...
    const animate = () => {
      this.minutesUntilTimeout = Math.floor(
        (Date.parse(this.sessionTimer) - Date.now()) / 1000 / 60
      );
      this.secondsUntilTimeout = Math.floor(
        ((Date.parse(this.sessionTimer) - Date.now()) / 1000) % 60
      );
      ...
      if (timer >= cutPoint50) {
        this.clipPath = `polygon(0% ${
          50 + this.percent(timer, top, cutPoint50)
        }%, 0% 100%, 50% 100%, 100% 100%, 100% 50%, 100% 0%, 50% 0%, 50% 50%)`;
      } else if (timer >= cutPoint40) {
        this.clipPath = `polygon(${
          0 + this.percent(timer, cutPoint50, cutPoint40)
        }% 100%, 50% 100%, 100% 100%, 100% 50%, 100% 0%, 50% 0%, 50% 50%)`;
      } else if (timer >= cutPoint30) {
        this.clipPath = `polygon(${
          50 + this.percent(timer, cutPoint40, cutPoint30)
        }% 100%, 100% 100%, 100% 50%, 100% 0%, 50% 0%, 50% 50%)`;
      } else if (timer >= cutPoint20) {
        this.clipPath = `polygon(100% ${
          100 - this.percent(timer, cutPoint30, cutPoint20)
        }%, 100% 50%, 100% 0%, 50% 0%, 50% 50%)`;
      } else if (timer >= cutPoint10) {
        this.clipPath = `polygon(100% ${
          50 - this.percent(timer, cutPoint20, cutPoint10)
        }%, 100% 0%, 50% 0%, 50% 50%)`;
      } else {
        this.clipPath = `polygon(${
          100 - this.percent(timer, cutPoint10, 0)
        }% 0%, 50% 0%, 50.0000000001% 50.0000000001%)`;
      }
